#ifndef DPS310SOFT_H_INCLUDED
#define DPS310SOFT_H_INCLUDED

#include "DpsClassSoft.h"
#include "util/dps310_config.h"

class Dps310Soft : public DpsClassSoft
{
public:
...
using namespace dps310soft;

int16_t Dps310Soft::getContResults(float *tempBuffer,
							   uint8_t &tempCount,
							   float *prsBuffer,
							   uint8_t &prsCount)
{
	return DpsClassSoft::getContResults(tempBuffer, tempCount, prsBuffer, prsCount, registers[FIFO_EMPTY]);
}

// #ifndef DPS_DISABLESPI
// int16_t Dps310Soft::setInterruptSources(uint8_t intr_source, uint8_t polarity)
// {
...
void Dps310Soft::init(void)
{
	int16_t prodId = readByteBitfield(registers[PROD_ID]);
	if (prodId < 0)
	{
		//Connected device is not a Dps310Soft
		m_initFail = 1U;
		return;
	}
	m_productID = prodId;
	...
	//make sure the DPS310Soft is in standby after initialization
	standby();
...
int16_t Dps310Soft::readcoeffs(void)
{
...
int16_t Dps310Soft::configTemp(uint8_t tempMr, uint8_t tempOsr)
{
	int16_t ret = DpsClassSoft::configTemp(tempMr, tempOsr);
...
int16_t Dps310Soft::configPressure(uint8_t prsMr, uint8_t prsOsr)
{
	int16_t ret = DpsClassSoft::configPressure(prsMr, prsOsr);
...
float Dps310Soft::calcTemp(int32_t raw)
{
	float temp = raw;
...
float Dps310Soft::calcPressure(int32_t raw)
{
	float prs = raw;
